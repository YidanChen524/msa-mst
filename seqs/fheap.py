...
class FibonacciHeap:
    ...
    def extract_min(self):
        ...
        self.consolidate()
        # return (seqs, vertex, key) of the old minNode
        return m.src, m.vertex, m.key

    def consolidate(self):
        # merge nodes with same degree on the root list
        len_arr = math.ceil(math.log(self.count+1, 2)) * 2
        arr = [None] * len_arr
    ...
    def decrease_key(self, src, node, key):
        ...
        # if success, set its seqs to seqs
    ...
    def decrease_all_connecting_keys(self, src):
        # decrease keys of all nodes connecting to seqs
